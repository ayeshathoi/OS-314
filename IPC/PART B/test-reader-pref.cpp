...
void *Reader(void* arg)
{
	...
	readerAcquireTime[threadNUmber] = index_cnt;
	index_cnt++;
	...
	pthread_spin_lock(&spinlock);
	readerReleaseTime[threadNUmber] = index_cnt;
	index_cnt++;
	pthread_spin_unlock(&spinlock);
...
void *Writer(void* arg)
{
  ...
  // Occupying the Lock
	...
	pthread_spin_lock(&spinlock);
	writerAcquireTime[threadNUmber] = index_cnt;
	index_cnt++;
	pthread_spin_unlock(&spinlock);
  ...
  usleep(10000);
	...
	pthread_spin_lock(&spinlock);
	writerReleaseTime[threadNUmber] = index_cnt;
	index_cnt++;
	pthread_spin_unlock(&spinlock);
